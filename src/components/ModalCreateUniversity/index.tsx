...
import { createUniversity, uploadImg } from "@/services";
...
const ModalCreateUniversity: React.FC<props> = ({ onClose, fetchData }) => {
  const { setLoading } = useLoading();
  const [form] = Form.useForm();
  const [imageUrl, setImageUrl] = useState<any>({});

  const handleCreateUniversity = async (values: any) => {
    try {
      setLoading(true);
...
export default ModalCreateUniversity;
